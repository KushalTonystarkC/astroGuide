...
import { geocode } from "@/lib/vedic/api"
import { cn } from "@/lib/utils"
import type { LocationChoice, NominatimResult } from "@/types/vedic-api"
...
export function CitySearch({
  ...
  return (
    <div className="relative space-y-2" ref={boxRef}>
      ...
      {open && results.length > 0 && (
        <ul
          ...
          className="absolute z-30 mt-1 max-h-64 w-full overflow-auto rounded-lg border border-border bg-popover py-1 shadow-md"
        >
          {results.map((r, i) => (
            <li
              key={r.place_id}
              ...
              onClick={() => choose(r)}
              className={cn(
                "cursor-pointer px-3 py-2 text-sm text-popover-foreground hover:bg-accent"
              )}
            >
              {r.display_name}
            </li>
