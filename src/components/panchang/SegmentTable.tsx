...
import { formatTime } from "@/lib/vedic/format";
import type { LabelledSegment } from "@/types/vedic-api";
...
export function SegmentTable({ segments, nameHeader, tz, testId, nameLookup }: Props) {
  ...
  return (
    <table className="w-full text-sm" data-testid={testId}>
      <thead>
        <tr className="text-ink-soft border-b border-parchment-200">
          <th className="py-1.5 pe-3 font-medium eyebrow text-start">{nameHeader}</th>
          <th className="py-1.5 pe-3 font-medium eyebrow num text-start">{t("col_time")}</th>
