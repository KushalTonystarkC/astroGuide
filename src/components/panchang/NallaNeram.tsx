...
import type { NallaNeramWindow } from "@/types/vedic-api";
    ...
    <table className="w-full text-sm" data-testid={testId}>
      <thead>
        <tr className="text-ink-soft border-b border-parchment-200">
          <th className="py-1.5 pe-3 font-medium eyebrow text-start">{t("col_planet")}</th>
          <th className="py-1.5 font-medium eyebrow num text-start">{t("col_time")}</th>
        </tr>
      </thead>
      <tbody>
        {windows.map((w, i) => (
